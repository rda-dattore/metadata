...
public:
  ...
  void warning(const SAXParseException& e) {
    ...
    parse_error+="Warning ("+strutils::itos(e.getLineNumber())+":"+strutils::itos(e.getColumnNumber())+"): "+string(msg);
  ...
  void error(const SAXParseException& e) {
    ...
    parse_error+="Error ("+strutils::itos(e.getLineNumber())+":"+strutils::itos(e.getColumnNumber())+"): "+string(msg);
    XMLString::release(&msg);
  }
  void fatalError(const SAXParseException& e) {
    ...
    parse_error+="Fatal error ("+strutils::itos(e.getLineNumber())+":"+strutils::itos(e.getColumnNumber())+"): "+string(msg);
    XMLString::release(&msg);
  }
  void resetErrors() {
  }
...
extern "C" void segv_handler(int) {
  ...
  mysystem2("/bin/tcsh -c \"/gpfs/u/home/dattore/bin/vm/dset_waf DBRESET\"",
      oss, ess);
  std::cout << "Database was reset for the next update" << endl;
}
...
void do_push(const std::deque<string>& arg_list) {
  ...
  for (const auto& repo : local_args.git_repos) {
    ...
    if (!ess.str().empty()) {
      if (exit_status == 0) {
        metautils::log_warning("git pull message: '" + ess.str() + "'; uflag "
            "was '" + uflag + "'", "dset_waf", user);
      } else {
...
void do_delete(const std::deque<string>& arg_list)
{
  std::vector<string> dsids;
  if (local_args.all_non_public) {
    Server server(metautils::directives.database_server,"metadata","metadata","rdadb");
    LocalQuery query("dsid","search.datasets","type not in ('P', 'H')");
    if (query.submit(server) < 0) {
      log_error("Database error while getting non-public datasets: '"+query.error()+"'","dset_waf",user);
      exit(1);
    }
  ...
  for (const auto& repo : local_args.git_repos) {
    ...
    for (const auto& dsid : dsids) {
      stringstream oss,ess;
      mysystem2("/bin/tcsh -c \"cd "+REPO_HEAD+"/"+repo+"; git rm ds"+dsid+".xml\"",oss,ess);
      if (!ess.str().empty()) {
        if (ess.str() != "fatal: pathspec 'ds"+dsid+".xml' did not match any files\n") {
    ...
    if (num_deleted > 0) {
      stringstream oss,ess;
      mysystem2("/bin/tcsh -c \"cd "+REPO_HEAD+"/"+repo+"; git pull -q\"",oss,ess);
      if (!ess.str().empty()) {
        log_error("git pull error: '"+ess.str()+"'","dset_waf",user);
      } else {
        mysystem2("/bin/tcsh -c \"cd "+REPO_HEAD+"/"+repo+"; git commit -m 'auto remove' -a\"",oss,ess);
        if (!ess.str().empty()) {
          log_error("git commit error: '"+ess.str()+"'","dset_waf",user);
        } else {
          mysystem2("/bin/tcsh -c \"cd "+REPO_HEAD+"/"+repo+"; git push -q\"",oss,ess);
          if (!ess.str().empty()) {
...
void do_db_reset(const std::deque<string>& arg_list)
{
  Server server(metautils::directives.database_server,"metadata","metadata","rdadb");
  LocalQuery query("distinct dsid","metautil.dset_waf");
  if (query.submit(server) < 0) {
    log_error("Database error while trying to fix a failed push: '"+query.error()+"'","dset_waf",user);
    exit(1);
  }
  if (server._delete("metautil.dset_waf") < 0) {
    log_error("Database error while trying to clear a failed push: '"+server.error()+"'","dset_waf",user);
    exit(1);
  }
  auto status=0;
  stringstream dslist;
  ...
  if (status < 0) {
    log_error("Database error while trying to clear a failed push - dataset list: '"+dslist.str()+"'","dset_waf",user);
    exit(1);
  }
}
...
int main(int argc, char **argv) {
  if (argc < 3 && (argc != 2 || string(argv[1]) != "DBRESET")) {
    cerr << "usage: dset_waf <action> [options...] <dslist>" << endl;
    cerr << "\nrequired:" << endl;
    ...
    cerr << "            \"nnn.n ...\" one or more individual dataset numbers" << endl;
    cerr << "\noptions:" << endl;
